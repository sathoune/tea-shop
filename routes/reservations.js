...
router.post("/", (req, res) => {
  ...
  Reservation.find({ date: dateCriteria }, (err, foundReservations) => {
    if (err) {
      console.error("error founding orders" + err);
    } else {
...
router.post("/new", (req, res) => {
  const newReservation = {
    date: new Date(
      `${req.body.day} ${req.body.hour - 1}:${req.body.minutes}:00 GMT+01:00`
    ),
    name: req.body.name,
  ...
  Reservation.create(newReservation, (err, createdReservation) => {
    if (err) {
      console.error("Not able to create reservation" + err);
    } else {
...
router.post("/update", (req, res) => {
  ...
  Reservation.findOneAndUpdate(
    ...
    (err, updatedReservation) => {
      if (err) {
        console.error("Not able to update reservation" + err);
      } else {
        res.send("updated");
      }
    }
...
router.post("/todo", (req, res) => {
  ...
  Reservation.find(
    { date: dateCriteria, done: false },
    (err, foundReservations) => {
      if (err) {
